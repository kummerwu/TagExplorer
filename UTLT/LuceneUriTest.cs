...
namespace UTLT
{
    ...
    public class LuceneUriTest
    {
        ...
        {
            db.AddUris(new List<string>() { @"c:\aaaa" }, new List<string>() { "tag1", "tag2" });
            db.AddUris(L(@"c:\a"),new List<string>() { "tag1", "tag2" });
        ...
        {
            db.AddUris(L(@"c:\a"), new List<string>() { "tag1", "tag2" });
            db.AddUris(L(@"c:\aaaa"), new List<string>() { "tag1", "tag2" });
        ...
        {
            db.AddUris(L(@"c:\a.txt"),new List<string>() { "tag1","tag2"});
        ...
        {
            db.AddUris(new List<string>() { @"c:\a.txt" }, new List<string>() { "tag1"});
        ...
        {
            db.AddUris(new List<string>() { @"D:\00_工作备份\Work\ROSng软件架构及应用V1.1.pptx" }, new List<string>() { "parent1" });
        ...
        public void TestUriMgr_Del()//测试删除文件，好像案例3测试不通过的原因就是删除失败了。
        {
            ...
            db.DelUris(new List<string>() { @"D:\00_工作备份\Work\ROSng软件架构及应用V1.1.pptx" }, false);
            ...
            //最终定位不是这个原因，是因为某个字段长度超过了50个字符，我们的解析器有问题（1-50 Ngram）
        {
            ...
            string tag = "child4";
            db.AddUris(new List<string>() { dir }, new List<string>() { tag });
            AssertIn(dir, tag);
            db.AddUris(new List<string>() { dir }, new List<string>() { tag });
            AssertIn(dir, tag);
        }
        ...
        {
            string uri = @"c:\a.txt";
            db.AddUriWithTitle(uri, new List<string>() { "tag1", "tag2" },"TITLE1");
        ...
        public void TestUriMgr_AddDel()
        {
            ...
            db.AddUriWithTitle(@"c:\a.txt", new List<string>() { "tag3", "tag4" }, "TITLE2");
            AssertIn(uri, "TITLE2","tag1", "tag2","tag3","tag4");



            db.DelTags(uri, new List<string>() { "tag1" });
            AssertNotIn("tag1");
            AssertIn(uri, "tag2", "tag3", "tag4");


            db.DelTags(uri, new List<string>() { "tag2" });
